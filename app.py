...
def detect_pullback(df: pd.DataFrame) -> Optional[dict]:
    ...
    # LONG: opened above SMA20, pulled down to touch it, bounced closed above it
    long_bounce = (curr["open"]  >  s20          # opened above — in uptrend
                   and curr["low"]   <= s20 * 1.002   # low touched SMA20
                   and curr["close"] >  s20)           # closed above — bounce confirmed

    # SHORT: opened below SMA20, pulled up to touch it, rejected closed below it
    short_bounce = (curr["open"]  <  s20          # opened below — in downtrend
                    and curr["high"]  >= s20 * 0.998  # high touched SMA20
                    and curr["close"] <  s20)          # closed below — rejection confirmed
...
def render_signal_card(row: dict):
    ...
    css = {"REVERSAL":"sig-reversal","EXPANSION":"sig-expansion","PULLBACK":"sig-pullback","COMPRESSION":"sig-compression"}.get(sig,"sig-expansion")
    ...
    badge = (f'<span class="badge-high">HIGH</span>'   if tier == "HIGH" else
             ...
             f'<span class="badge-watch">WATCH</span>'   if tier == "WATCH" else
