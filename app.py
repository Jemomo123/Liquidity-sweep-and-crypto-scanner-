...
def render_signal_card(row: dict):
    ...
    if sig == "CHOP":
        ...
        # Format reasons in plain English
        reasons_plain = []
        for r in chop_reasons:
            if "Flat SMAs" in r or "Weak SMA trend" in r:
                reasons_plain.append(f"📉 {r} — Moving averages going sideways, not trending")
            elif "whipsaw" in r or "crosses" in r:
                reasons_plain.append(f"🔀 {r} — Price bouncing above/below SMAs repeatedly")
            elif "Volatility" in r:
                reasons_plain.append(f"💤 {r} — Price range shrinking, market getting quieter")
            else:
                reasons_plain.append(f"• {r}")
        
        reasons_html = "<br>".join(reasons_plain)
        
        html = f"""
        <div style="background:{color_bg};border:2px solid {color_border};border-left:6px solid {color_border};
                    border-radius:10px;padding:16px 18px;margin-bottom:12px;">
            <div style="display:grid;grid-template-columns:2.5fr 1fr;gap:16px;">
                <div>
                    <div style="font-weight:900;font-size:1.05rem;color:{color_text};margin-bottom:4px;">
                        ⚠️ CHOP MODE - {row.get('pair','')}
                    </div>
                    <div style="font-size:0.7rem;color:{color_text};opacity:0.8;margin-bottom:12px;">
                        {row.get('tf','')} · {row.get('source','').upper()} · ${row.get('price','')}
                    </div>
                    
                    <div style="background:#fff;padding:10px 12px;border-radius:6px;margin-bottom:12px;
                                border-left:3px solid {color_border};">
                        <div style="font-size:0.85rem;font-weight:700;color:{color_text};margin-bottom:8px;">
                            {interpretation}
                        </div>
                    ...
                    <div style="font-size:0.82rem;font-weight:800;color:{color_text};background:#fff;
                                padding:10px 12px;border-radius:6px;text-align:center;">
                        {advice} — Wait for breakout or trade different pairs
                    </div>
                </div>
                <div style="text-align:center;padding-top:10px;">
                    <div style="font-size:0.7rem;color:{color_text};opacity:0.8;margin-bottom:6px;
                                text-transform:uppercase;letter-spacing:0.5px;">Chop Score</div>
                    <div style="font-size:2.8rem;font-weight:900;color:{color_text};line-height:1;">
                        {chop_score}
                    </div>
                    <div style="font-size:0.75rem;color:{color_text};opacity:0.8;margin-bottom:12px;">/100</div>
                    <div style="font-size:0.7rem;color:{color_text};opacity:0.7;line-height:1.5;">
                        {"Extremely choppy" if chop_score >= 75 else "Very choppy" if chop_score >= 60 else "Choppy"}
                    </div>
                </div>
            </div>
        </div>
