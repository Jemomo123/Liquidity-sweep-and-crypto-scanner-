...
def scan_pair(pair: str, source: str, tf: str) -> Optional[dict]:
    try:
        ...
        # PRIORITY 1: Report CHOP if detected (prevents false signals)
        if chop["is_chop"]:
            disp = pair_display(pair, source)
            add_log(source.upper(), disp, "CHOP", f"Score:{chop['chop_score']}", "AVOID")
            return {
                "pair":          disp,
                "raw_pair":      pair,
                "source":        source,
                ...
                "obs":           "SMAs flat",
                "obs_d":         "",
                "freshness":     f"{chop['crosses']} crosses",
                "signal_age":    0,
                "comp_state":    "CHOP",
        ...
        if not signal_type and comp["state"] != "NONE":
            ...
            return {
                ...
                "score":         0,
                "tier":          "WATCH",
                "parts":         [f"{quality_emoji} {quality} quality · {comp.get('spread_pct','—')}% spread"],
                "quality":       quality,
                "bias_15m":      "—",
                "rsi":           round(rsi, 1) if not pd.isna(rsi) else "—",
                "room":          "—",
