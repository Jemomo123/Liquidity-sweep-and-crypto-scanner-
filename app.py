...
def get_btc_open_interest_state() -> dict:
    """
    Track BTC Open Interest with AUTO-FAILOVER across exchanges.
    Tries: OKX → Gate.io → Bybit → Binance
    Returns current OI + directs to Coinglass for trends.
    """
    import requests
    
    exchanges = [
        {"name": "OKX", "url": "https://www.okx.com/api/v5/public/open-interest", "param": "instId", "symbol": "BTC-USDT-SWAP"},
        {"name": "Gate.io", "url": "https://api.gateio.ws/api/v4/futures/usdt/contracts/BTC_USDT", "param": None},
        {"name": "Bybit", "url": "https://api.bybit.com/v5/market/open-interest", "param": "symbol", "symbol": "BTCUSDT"},
        {"name": "Binance", "url": "https://fapi.binance.com/fapi/v1/openInterest", "param": "symbol", "symbol": "BTCUSDT"}
    ]
    
    for exch in exchanges:
        try:
            params = {exch["param"]: exch["symbol"]} if exch["param"] else {}
            ...
            elif exch["name"] == "Bybit":
                oi = float(data.get("result", {}).get("list", [{}])[0].get("openInterest", 0))
            elif exch["name"] == "Binance":
            ...
            if oi > 0:
                return {
                    ...
                    "oi_24h_change_pct": 0,
                    "direction": "Visit Coinglass",
                    "status": "→ Check trends manually",
                    "funding_rate": 0,
                    "price_24h_change_pct": 0,
                    "setup_detected": "Manual",
                    "price_trend": "—",
    ...
    return {"error": "All exchanges failed", "data_source": "None"}
...
def render_regime(regime: dict):
    ...
    try:
        squeeze = detect_squeeze_setup("BTC/USDT", "OKX", regime)
        if squeeze.get("type"):
            if squeeze["type"] == "short_squeeze":
                st.success(f"""
                    **🎯 SHORT SQUEEZE SETUP DETECTED** ({squeeze['confidence']}% confidence)
                    
                    **Action:** Wait for bottom, then LONG
                    
                    **Expected Move:** {squeeze['expected_move']}
                    
                    **Signals:** {' · '.join(squeeze['signals'])}
                    
                    **RSI:** {squeeze['rsi']:.1f} · **12H Change:** {squeeze['price_change']:+.2f}%
                """)
            elif squeeze["type"] == "long_squeeze":
                st.error(f"""
                    **⚠️ LONG SQUEEZE SETUP DETECTED** ({squeeze['confidence']}% confidence)
                    
                    **Action:** Wait for top, then SHORT
        ...
        pass  # Silently fail if squeeze detection has issues
...
def render_liquidity(panel: dict):
    ...
    st.markdown('<div style="font-size:0.9rem;font-weight:800;color:#111;margin-bottom:4px;">📊 Open Interest & Squeeze Detector</div>', unsafe_allow_html=True)
    ...
    if oi_data and not oi_data.get("error"):
        # Show which exchange provided data
        data_source = oi_data.get("data_source", "Unknown")
        st.markdown(f'<div style="font-size:0.7rem;color:#888;margin-bottom:8px;">Data: {data_source}</div>', unsafe_allow_html=True)
