...
def render_liquidity(panel: dict):
    ...
    if not oi_data.get("error"):
        ...
        with col1:
            st.metric(
                label="Current OI",
                value=f"{oi_data['current_oi']:,.0f} BTC",
                delta=f"{oi_data['oi_24h_change_pct']:+.1f}% (24h)"
            )
        
        with col2:
            st.metric(
                label="OI Direction",
                value=oi_data['direction'],
                delta=oi_data['status']
            )
        ...
        with col4:
            price_change = oi_data['price_24h_change_pct']
            st.metric(
                label="Price 24H",
                value=f"{price_change:+.2f}%",
                delta=oi_data['price_trend'].title()
            )
        
        # Squeeze Setup Alert
        setup = oi_data['setup_detected']
        if setup != "None" and "Error" not in setup:
