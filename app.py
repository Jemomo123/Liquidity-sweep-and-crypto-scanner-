...
def detect_compression_state(df: pd.DataFrame) -> dict:
    ...
    # Detect CROSSOVER:
    # Either SMA20 crossed SMA100 this candle (classic crossover)
    # OR SMA20 and SMA100 are extremely close (within 0.05%) while in compression
    # — this catches V-shapes and single-candle meets where they don't literally cross
    is_cross = False
    if not any(pd.isna(x) for x in [ps20, ps100, s20, s100]):
        ...
        already_together = sma_gap <= 0.05  # SMA20 and SMA100 within 0.05% = together
        is_cross = crossed_this_candle or already_together

    if is_cross:
        # CROSSOVER — report on 1 candle
        state = "CROSSOVER"
        compression_active = True
    elif candles_in >= COMPRESSION_MIN_CANDLES:
        # SQZ — 3+ consecutive candles
        state = "SQZ"
        compression_active = True
    else:
        # Building but not yet active — stay silent
        return {
            "state": "NONE",
            "spread_pct": round(current_spread, 3),
            "compression_active": False,
