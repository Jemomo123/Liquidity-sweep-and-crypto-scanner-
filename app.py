...
def get_btc_session_performance(lookback_days: int = 30) -> dict:
    ...
    try:
        ...
        if 'timestamp' not in df.columns and hasattr(df.index, 'to_pydatetime'):
            df['timestamp'] = pd.to_datetime(df.index)
        elif 'timestamp' in df.columns:
            df['timestamp'] = pd.to_datetime(df['timestamp'], utc=True, errors='coerce')
        else:
            # Fallback: create timestamps from current time backwards
            from datetime import datetime, timedelta, timezone
            now = datetime.now(timezone.utc)
            timestamps = [now - timedelta(hours=i) for i in range(len(df)-1, -1, -1)]
        ...
        try:
            ...
            # Fallback: use index position modulo 24
            df['hour_utc'] = [i % 24 for i in range(len(df))]
